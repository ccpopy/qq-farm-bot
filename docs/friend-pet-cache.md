...
300–500 位好友共用一轮列表读取和访问计划，每次只向账号队列提交一位好友。整轮由后台任务推进，农场巡查和手动操作可以在好友访问之间执行；好友检查间隔仍表示整轮结束后到下一轮开始的等待时间。单次访问内部仍保持 Enter、操作、Leave 连续执行，因此长耗时的单次访问仍会影响其他账号任务的等待时间。

每次进入前重读自动化开关、黑名单、帮助经验和捣乱额度。停止、进入静默时段或跨日会结束旧轮次；网关退避保留当前访问位置，等待期间不占用账号队列。每访问 50 位记录一次进度，正常完成时记录总耗时。

自动巡查、手动操作、宠物同步、天气和互动道具统一使用 `withFriendFarmVisit()`。该函数沿用账号队列的嵌套调用机制，不额外加锁；成功进入后始终尝试 Leave，后台访问的 Leave 使用正常优先级及 `friend` 请求类别。

`core/tests/friend-scheduling.test.js` 使用 500 位模拟好友验证单次列表读取和任务交替执行，并覆盖动态配置、跨日、退避续跑、取消和访问异常。模拟测试不代表真实网关吞吐量。
