...
test('a failed farm read records the farm check as an error while preserving the boolean API', async (t) => {
    const runner = require('../dist/app/account-task-runner');
    const store = require('../dist/models/store');
    const network = require('../dist/utils/network');
    const farmApi = require('../dist/services/farm/api');
    ...
    assert.equal(metrics.find(metric => metric.name === 'farm.phase.get-lands').outcome, 'error');
    assert.equal(metrics.find(metric => metric.name === 'farm.check').outcome, 'error');
});
