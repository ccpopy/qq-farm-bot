...
let qqPollKey = ''
let qqCreateInFlight: Promise<any> | undefined
let qqCancelInFlight: Promise<void> | undefined
...
function resetQqLogin() {
  ...
  stopQqPolling()
  if (taskId)
    qqCancelInFlight = cancelQqLoginTask(taskId)
  qqTaskId.value = ''
  qqStatus.value = ''
  qqError.value = ''
  qqQrUrl.value = ''
...
async function startQqLogin() {
  ...
  try {
    // Wait for an earlier create/cancel cycle so NapCat can release its login workflow.
    await qqCreateInFlight?.catch(() => undefined)
    await qqCancelInFlight
    if (flowVersion !== qqFlowVersion)
      return
    request = api.post('/api/qq-login/tasks', undefined, { timeout: 120000, skipErrorToast: true } as any)
    qqCreateInFlight = request
    ...
    if (qqCreateInFlight === request)
      qqCreateInFlight = undefined
    if (flowVersion === qqFlowVersion)
      qqLoading.value = false
  }
}
